...
/*
	...
	//===============================================================================================
	4-Bit-Modus (Anschluss):
	----------------------------------
	1) GND (VSS) - Masse
	2) VDD - +5V Spannungsversorgung
	3) V0 - Kontrastregelung (über Potentiometer von 0 bis 5 Volt)
	4) RS - Registerauswahl (Befehle oder Daten) rs = 1 für Daten, rs = 0 für Befehl
	5) R/W - Lesen/Schreiben (Schreibmodus bei Masseanschluss). Wenn nichts vom Display gelesen werden soll, Masseanschluss.
	6) E - Aktivieren (Strobe bei fallender Flanke)
	7) DB0 - Datenleitung (nicht im 4-Bit-Modus verwendet)
	8) DB1 - Datenleitung (nicht im 4-Bit-Modus verwendet)
	9) DB2 - Datenleitung (nicht im 4-Bit-Modus verwendet)
	10) DB3 - Datenleitung (nicht im 4-Bit-Modus verwendet)
	11) DB4 - Datenleitung (wird im 4-Bit-Modus verwendet)
	12) DB5 - Datenleitung (wird im 4-Bit-Modus)
	13) DB6 – Datenleitung (wird im 4-Bit-Modus verwendet)
	14) DB7 – Datenleitung (wird im 4-Bit-Modus verwendet)
	15) A – Hintergrundbeleuchtung positiv (+5 Volt oder PWM)
	16) K – Hintergrundbeleuchtung negativ

	Das Display selbst kann in zwei Modi betrieben werden:
	• 8-Bit-Modus – nutzt sowohl die niederwertigen als auch die höherwertigen Bits (DB0–DB7)
	• 4-Bit-Modus – nutzt nur die niederwertigen Bits (DB4–DB7)

	Der 8-Bit-Modus ist bei diesem Display unpraktisch.

	Er benötigt vier zusätzliche Pins, und der Geschwindigkeitsgewinn ist praktisch nicht vorhanden, da die 	
	Bildwiederholfrequenz dieses Displays auf <10 Mal pro Sekunde begrenzt ist.
*/

		// выбираем через какой интефейс работает дисплей: --------------------------------------------------
		// ( нужное разкомментировать )
		// wählen Sie die Schnittstelle aus, über die das Display funktioniert: -----------------------------
		// (kommentieren Sie aus/ein, was Sie benötigen)
		#define LCD1602_I2C			// режим I2C - I2C Modus
		//#define LCD1602_4BIT			// режим 4BIT - 4BIT Modus
		//---------------------------------------------------------------------------------------------------
		
		
		#if defined (LCD1602_I2C)		// если выбран режим I2C ( то указываем адрес шины и какой I2C используем )--
							// Wenn der I2C-Modus ausgewählt ist (dann geben wir die Busadresse und den 
							// verwendeten I2C-Typ an)--
		...
		// Anmerkung HP: Nicht verwenden, unsere Displays haben keinen kyrillischen 2. Zeichensatz,
		// sondern einen chinesischen...
